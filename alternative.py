...
def specialProcess(input_csv='EmbeddingStore.csv', output_csv='EmbeddingValuesMap.csv', k=0.5, approach=1):
...
class HalfDimOpenAIEmbedder(BaseOpenAIEmbedder):
    def __init__(self, model_name, batch_size=16):
        super().__init__(model_name, batch_size, dimensions=None)
        self.base_dimensions = 1536
        self.half_dimensions = self.base_dimensions // 2

    def _get_embeddings(self, texts):
        kwargs = {
            "model": self.model_name,
            "input": texts,
            "dimensions": self.half_dimensions  # Request half the dimensions from the API 
        }
...
for task in tasks:
    ...
    custom_results = benchmark.run(custom_model, output_folder=f"Result/OpenAI/Custom_1/{model_name}_k_05")

# 2. Normal run: no modifications
for task in tasks:
    benchmark = MTEB(tasks=[task])
    normal_model = NormalOpenAIEmbedder(model_name=model_name, batch_size=16)
    normal_results = benchmark.run(normal_model, output_folder=f"Result/OpenAI/Normal/{model_name}")

# 3. Half-Dimension run: Use dimensions parameter directly (MRL)
for task in tasks:
    benchmark = MTEB(tasks=[task])
    halfdim_model = HalfDimOpenAIEmbedder(model_name=model_name, batch_size=16)
    halfdim_results = benchmark.run(halfdim_model, output_folder=f"Result/OpenAI/HalfDim/{model_name}")
